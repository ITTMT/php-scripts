...
<div>
    ...
    if (isset($_POST['login'])) {
        ...
        $check_ip = $con->prepare("SELECT ip_id, ip_timeout FROM ips WHERE ip_ip = ? LIMIT 1");
        ...
        $test = $con-> prepare("INSERT INTO ips(ip_ip, ip_timeout) VALUES(?, ?)");
        $now = new DateTime();
        $now->modify("+2 minutes");

        $test->bind_param("ss", $ip, $now);
        $test->execute();
